...
class Part3Test {  // ======== 16 points ========
    ...
    void BGBlobSize3() {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                sum += red.undiscoveredBlobSize(i, j, b.flatten(), visited.clone());
                sum += yellow.undiscoveredBlobSize(i, j, b.flatten(), visited.clone());
                sum += blue.undiscoveredBlobSize(i, j, b.flatten(), visited.clone());
                sum += green.undiscoveredBlobSize(i, j, b.flatten(), visited.clone());
                visited[i][j] = true;
            }
        }
